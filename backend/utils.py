...
def generate_excel(db: Session) -> BytesIO:
    ...
    for grade in grades:
            ...
            for class_ in classes:
                ...
                if len(dorms) > 0:
                    ws.merge_cells(
                        start_row=row_index,
                        start_column=1,
                        end_row=row_index + len(dorms) - 1,
                        end_column=1,
                    )
                    ws.cell(
                        row=row_index, column=1
                    ).value = f"{grade_name}{major.MajorName}{class_.ClassName}"
                ...
                for i, dorm in enumerate(dorms):
                    ...
                    sanitation_records = (
                        db.query(Sanitation)
                        .filter(Sanitation.DormID == dorm.DormID)
                        .all()
                    )
                    ...
                    for sanitation in sanitation_records:
                        ...
                        if sanitation.Status == "优秀":
                            excellent_counts[week_number] = (
                                excellent_counts.get(week_number, 0) + 1
                            )

                        # 将 weekday_str 转换为数字索引，假设 "周一" 对应 0，"周二" 对应 1，以此类推
                        weekday_to_index = {1: 0, 2: 1, 3: 2, 4: 3, 5: 4}
                        weekday = weekday_to_index.get(
                            sanitation.Weekday, -1
                        )  # 如果找不到对应的索引，返回 -1

                        # 计算应填充到的单元格的列索引
                        col_index_for_week = (
                            4 + (week_number - 1) * 6
                        )  # 从第4列开始，每周有6列（包括“优秀次数”列）
                        col_index_for_status = (
                            col_index_for_week + weekday
                        )  # 根据 Weekday 的数字索引来确定
                    ...
                    for week_number, count in excellent_counts.items():
                        col_index_for_excellent = (
                            3 + (week_number - 1) * 6
                        )  # 从第3列开始，每周有6列（包括“优秀次数”列）
                        ws.cell(
                            row=row_index + i, column=col_index_for_excellent
                        ).value = count
        ...
        for week in range(1, 17):
            # 合并单元格
            ws.merge_cells(
                start_row=1, start_column=col_index, end_row=1, end_column=col_index + 4
            )
            ws.merge_cells(
                start_row=1,
...
class ResponseBase(BaseModel):
    code: CodeEnum = Field(default=CodeEnum.SUCCESS, description="业务代码：200成功，400失败")
